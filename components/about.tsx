...
export function About() {
  return (
    <section id="introduccion" className="bg-paper py-24 sm:py-32">
      <div className="section-shell grid min-w-0 items-center gap-14 lg:grid-cols-[1fr_.86fr] lg:gap-24">
        ...
        <Reveal delay={0.12} className="relative min-w-0 overflow-hidden sm:overflow-visible"><div className="relative overflow-hidden rounded-[2.4rem] bg-secondary p-3 shadow-soft sm:p-4"><div className="relative aspect-[4/5] overflow-hidden rounded-[1.9rem]"><Image src="/images/intro-aprender-haciendo.png" alt="Scout aprendiendo a preparar alimentos en una actividad al aire libre" fill className="object-cover" sizes="(max-width: 1024px) 90vw, 42vw" /></div><div className="absolute bottom-8 left-8 right-8 flex items-center gap-3 rounded-2xl bg-paper/95 p-4 shadow-card backdrop-blur"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-accent text-ink"><BookOpenCheck size={19} /></span><div className="min-w-0"><p className="text-xs font-extrabold">Aprender haciendo</p><p className="mt-0.5 text-[11px] text-ink/55">El territorio también es aula.</p></div></div></div><div className="absolute -right-5 -top-5 -z-0 hidden h-24 w-24 rounded-full border-[14px] border-accent sm:block sm:-right-8 sm:-top-8" /></Reveal>
      </div>
    </section>
  );
}
