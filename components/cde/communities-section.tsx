...
export function CommunitiesSection({ cde }: { cde: CDEData }) {
  return <section id="comunidad" className="scroll-mt-36 bg-mist py-20 sm:py-28"><div className="section-shell"><Reveal><SectionTitle eyebrow="02 · Comunidades" title={<>Una ruta común,<br /><span className="text-secondary">distintas edades.</span></>} description="Del juego y el cuidado en Ronda a los proyectos autónomos del Clan, cada comunidad tiene una etapa, una forma de organizarse y una ruta de aprendizaje propia." /></Reveal>{cde.communities.length > 0 ? <div className="mt-14 grid gap-5 sm:grid-cols-2">{cde.communities.map((community, index) => <Reveal key={community.id} delay={index * 0.06}><CommunityCard cdeSlug={cde.slug} community={community} /></Reveal>)}</div> : <div className="mt-14 rounded-[2rem] border border-dashed border-ink/20 bg-paper p-10 text-center"><UsersRound className="mx-auto text-primary" size={34} /><p className="mt-4 text-sm font-bold text-ink/60">Este CDE todavía no ha configurado sus comunidades.</p></div>}</div></section>;
}
