      ...
      <div className="section-shell">
        <div className="rounded-[1.8rem] border border-white/15 bg-white/[0.06] p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,1fr))]">
            <label className="block text-sm font-bold text-white">Buscar<span className="relative mt-2 block"><Search size={17} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-white/45" aria-hidden="true" /><input id="blog-search" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Título, autor o palabra clave" className="focus-ring h-12 w-full rounded-xl border border-white/20 bg-white/10 pl-10 pr-3 text-base font-medium text-white outline-none placeholder:text-white/45 sm:text-sm" /></span></label>
            <label className="block text-sm font-bold text-white">Categoría<select value={category} onChange={(event) => { setCategory(event.target.value); setPage(1); }} className="focus-ring mt-2 h-12 w-full rounded-xl border border-white/20 bg-ink px-3 text-base font-medium text-white outline-none sm:text-sm">{categories.map((option) => <option key={option}>{option}</option>)}</select></label>
            <label className="block text-sm font-bold text-white">Etiqueta<select value={tag} onChange={(event) => { setTag(event.target.value); setPage(1); }} className="focus-ring mt-2 h-12 w-full rounded-xl border border-white/20 bg-ink px-3 text-base font-medium text-white outline-none sm:text-sm">{tags.map((option) => <option key={option}>{option}</option>)}</select></label>
            <label className="block text-sm font-bold text-white">Ordenar<select value={sort} onChange={(event) => { setSort(event.target.value as SortOption); setPage(1); }} className="focus-ring mt-2 h-12 w-full rounded-xl border border-white/20 bg-ink px-3 text-base font-medium text-white outline-none sm:text-sm"><option value="recent">Más recientes</option><option value="oldest">Más antiguas</option><option value="title">Título A–Z</option></select></label>
          </div>
        </div>
