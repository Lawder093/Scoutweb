  ...
  return (
    <section id="biblioteca" className="bg-paper py-24 sm:py-32">
      <div className="section-shell">
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 sm:gap-5 md:grid-cols-3">{resources.slice(0, 6).map((resource, index) => <Reveal key={resource.id} delay={index * 0.05}><LibraryResourceCard resource={resource} compact /></Reveal>)}</div>
      </div>
    </section>
  );
}
